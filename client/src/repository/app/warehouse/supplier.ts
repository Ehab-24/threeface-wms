...
export async function getAll(): Promise<Supplier[] | null> {
    try {
...
export async function updateSupplier(supplier: Supplier): Promise<Supplier | null> {
    try {
        const {data} = await appAPI.patch('/warehouse/supplier/' + supplier._id, supplier);
        return data.data as Supplier;
    }
    catch (error: any) {
        alert(error.response.data?.message ?? error.message);
