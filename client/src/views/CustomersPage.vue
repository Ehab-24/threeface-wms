...
customerRepository.getAll(10, 1).then((data) => {
  if (data) {
...
<template>
  ...
  <div class="mt-4 grid grid-flow-row-dense  w-max">
    <DataTable @click="openModal"
      :headers="['name', 'Last Purchase', 'Total Purchases', 'total Spent', 'created at']"
      :data-records="customers" />
  </div>

  <Modal ref="modalRef">
    <AddCustomerForm @add-customer="(customer: Customer) => customers.push(customer)" />
  </Modal>
</template>